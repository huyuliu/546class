...
```{r}
getwd()
setwd("/Users/huyusmac/Documents/class/BCB546X/546class/R_assignment") ##This WD does not work with anone else who would use this code on their computer
file.info("fang_et_al_genotypes.txt") ##It would be nice to read the files from online instead of loading them into the WD.
...
maize_all <- maize_all[c(14,16,1:13,15,17:985),] ##It might be good to have more explination of lines as you go along.
...
for(thisName in allNames[c(1:10)]){
     ...
     write.table(splitData[[thisName]], file = saveName, sep = "\t", row.names = FALSE) ##Could make the file path so that it writes directly into your maize (And later teosinte) file dirs
}
```
...
genotype2 <- data.frame(sapply(genotype2,function(x) {x <- gsub("\\w/\\w","hete",x)}))#Nice use of applying a function
...
ggplot(data=maize_vs_teosinte, mapping=aes(x=Group, y=sum_count, fill=genotype)) + geom_bar(stat="identity", position = 'stack')+ geom_text(mapping = aes(label = sum_count),size = 5, colour = 'black',  position = position_stack()) ##This is a cool graph to compare the two species!
```
...
#CNC review notes - It would have been nice to have this project as a seperate repositiory so that I didn't have to load in your whole repro for the class.
